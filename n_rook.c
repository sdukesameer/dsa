...
int rooks(int arr[N][N], int n){//n is col no, initially its 0
    if(n>=N)//best case, when all rooks are well placed
        return 1;
    for (int i = 0; i < N; i++){//surf accross rows
        if(safe(arr,i,n)){
            arr[i][n]=1;//if safe insert a rook
            if(rooks(arr,n+1))//then check if at next col its safe to place rook
                return 1;
...
int main(){
    ...
    for (int i = 0; i < N; i++)//initiaise all to 0
        for (int j = 0; j < N; j++)
            arr[i][j]=0;
    int ans=rooks(arr,0);//rooks function call
